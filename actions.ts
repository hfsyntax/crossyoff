...
const handleSubmitRun = async (prevState: any, formData: FormData): Promise<HighscoreFormResult> => {
    ...
    const scoreQueryResult = await sql`SELECT score FROM crossy_road_records WHERE platform = ${platform} AND score < ${score}`
    ...
    await fetch(webhookURL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(discordEmbed) })
        .catch(error => {
            console.error(error)
